...
def get_single_file_description(file_path: str, question: str, visual_inspection_tool, document_inspection_tool):
    # Fix file path resolution - prioritize data/gaia/2023/ path
    ...
    if not os.path.exists(file_path):
        ...
        if os.path.exists(alt_path):
            ...
            print(f"Warning: Cannot find file at {file_path} or {alt_path}")
            return f" - Attached file: {file_path} (file not found)"
    
    file_extension = actual_file_path.split(".")[-1]
    if file_extension in ["png", "jpg", "jpeg"]:
        file_description = f" - Attached image: {file_path}"
        file_description += (
            f"\n     -> Image description: {get_image_description(actual_file_path, question, visual_inspection_tool)}"
        )
        return file_description
    elif file_extension in ["pdf", "xls", "xlsx", "docx", "doc", "xml"]:
        image_path = actual_file_path.split(".")[0] + ".png"
        if os.path.exists(image_path):
            description = get_image_description(image_path, question, visual_inspection_tool)
            display_path = image_path
        else:
            description = get_document_description(actual_file_path, question, document_inspection_tool)
            display_path = actual_file_path
...
def get_zip_description(file_path: str, question: str, visual_inspection_tool, document_inspection_tool):
    ...
    if not os.path.exists(file_path):
        ...
        if os.path.exists(alt_path):
            ...
            print(f"Warning: Cannot find zip file at {file_path} or {alt_path}")
            return f" - Attached file: {file_path} (file not found)"
    
    try:
        shutil.unpack_archive(actual_file_path, folder_path)
    except Exception as e:
        print(f"Error unpacking {actual_file_path}: {e}")
        return f" - Attached file: {file_path} (extraction failed: {e})"
